...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.news.store') }}" enctype="multipart/form-data" style="width: 600px;">
        ...
        <div class="form-group">
            ...
            <input class="form-control" id="title" value=" {{ old('title') }}" name="title" aria-required="true">
        </div>
        <div class="form-group">
            <label class="control-label" for="description">Текст новости</label>
            <textarea class="form-control" id="description" name="description" aria-required="true">{{ old('text') }}</textarea>
        </div>
        <div class="form-group">
            <label class="control-label" for="author">Автор новости</label>
            <input class="form-control" id="author" value=" {{ old('author') }}" name="author" aria-required="true">
        </div>
        <div class="form-group">
